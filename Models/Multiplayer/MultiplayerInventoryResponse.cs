...
namespace MCE_API_SERVER.Models.Multiplayer
{
	...
	public class MultiplayerItem
	{
		...
		public static MultiplayerItem ConvertToMultiplayerItem(InventoryResponse.Hotbar item)
		{
            ...
            MultiplayerItem multiplayerItem = new MultiplayerItem
			{
				...
				owned = false,
				rarity = new MultiplayerItemRarity { loc = ItemRarity.Invalid, value = 6 }
			};
		...
		{
            Features.Item catalogItem = StateSingleton.catalog.result.items.Find(match => match.id == itemId);
			ItemCategory category =
				Enum.TryParse(catalogItem.category, true, out ItemCategory itemCategory)
					? itemCategory
					: ItemCategory.Invalid;

			ItemRarity rarity = Enum.TryParse(Enum.GetName(typeof(Item.Rarity), catalogItem.rarity), true, out ItemRarity itemRarity)
				? itemRarity
				: ItemRarity.Invalid;
            ...
            MultiplayerItem multiplayerItem = new MultiplayerItem
			{
				...
				guid = itemId,
				owned = owned,
				category = new MultiplayerItemCategory { loc = category, value = (int)category },
				rarity = new MultiplayerItemRarity { loc = rarity, value = (int)rarity }
			};

			if (unlockedOn != null) {
				multiplayerItem.unlocked = new MultiplayerItemUnlocked { on = EpochTime.GetIntDate(unlockedOn.Value) };
			}
